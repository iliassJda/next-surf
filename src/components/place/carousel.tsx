  ...
  useEffect(() => {
    const getImages = async () => {
      try {
        ...
        const imageUrls = spots.map(
          (spot: { imageURL: string }) => spot.imageURL
        );

        const titles = spots.map((spot: { title: string }) => spot.title);
        const countries = spots.map(
          (spot: { country: string }) => spot.country
        );
        const cities = spots.map((spot: { city: string }) => spot.city);
        const longitudes = spots.map(
          (spot: { longitude: number }) => spot.longitude
        );
        const latitudes = spots.map(
          (spot: { latitude: number }) => spot.latitude
        );
          ...
          >
            {images.map((imageURL, idx) => (
              <Carousel.Item key={idx} className={styles.carouselItem}>
                <Link
                  href={`/places/${countries[idx]}/${cities[idx]}/${titles[idx]}/${longitudes[idx]}/${latitudes[idx]}`}
                >
                  <div className={styles.imageContainer}>
                    ...
                    <div className={styles.title}>
                      ...
                      <p className={styles.paragraphCountry}>
                        {countries[idx]}
                      </p>
                    </div>
                  </div>
